...
|---|---|---|---|
| Public site | Available | `npm run start`, then `/` | Static Terajs DOM site served from built assets; backend state remains local. |
...
| Local MCP engine | Available | `npm run truth` or installed `quotarelay-mcp --cli truth` | Agent-facing stdio MCP tool; no native desktop/mobile app or hosted tenancy. |
